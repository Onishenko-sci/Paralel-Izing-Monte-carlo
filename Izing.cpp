...
void simulate(size_t thread_id, Izing &izing, unsigned int steps, Barrier &bar)
{
    // Init distributions for spins
    std::random_device seed_init;
    std::mt19937 gen(seed_init());
    ...
    for (unsigned long int step = 0; step < steps; step++)
    {
     ...
     //       cout << thread_id << std::flush;
        ...
        if (izing.Frame_rate && step % izing.Frame_rate == 0)
            bar.Wait();
        //   izing.observed.lock();
        //   izing.my_step[thread_id] = step;
        //   if (thread_id == 0)
        //       for (int i = 0; i <= Last_thread_id; i++)
        //           izing.thread_relative_step[i][step] = int(izing.my_step[i]);
        //   izing.observed.unlock();
...
void Izing::simulation(unsigned long int steps, double kT_in, int frame_rate, int number_of_threads)
{
    unsigned int start_time = clock();
    // Check number_of_threads
    int max_threads = thread::hardware_concurrency();
    ...
    Mutexi = new mutex[Number_of_threads - 1];
    for (int i = 1; i < Number_of_threads; i++)
        threads.push_back(thread(simulate, i, std::ref(*this), steps, std::ref(Bar)));
    // Use main as 0 thread
    simulate(0, *this, steps, Bar);
    // Join threads
    for (int i = 0; i < Number_of_threads - 1; i++)
        threads[i].join();
  //  cout << std::endl;
...
void simulate_block(size_t thread_id, Izing &izing, unsigned int steps, Barrier &bar)
{
    // Init distributions for spins
    std::random_device seed_init;
    std::mt19937 gen(seed_init());
...
void Izing::simulation_block(unsigned long int steps, double kT_in, int frame_rate, int number_of_threads)
{
    unsigned int start_time = clock();
    // Check number_of_threads
    int max_threads = thread::hardware_concurrency();
    ...
    Mutexi = new mutex[Number_of_threads - 1];
    for (int i = 1; i < Number_of_threads; i++)
        threads.push_back(thread(simulate_block, i, std::ref(*this), steps, std::ref(Bar)));
    // Use main as 0 thread
    simulate_block(0, *this, steps, Bar);
    // Join threads
    for (int i = 0; i < Number_of_threads - 1; i++)
        threads[i].join();
   // cout << std::endl;
...
void checkmate(size_t thread_id, Izing &izing, unsigned int steps, Barrier &bar)
{
    std::random_device seed_init;
    std::mt19937 gen(seed_init());
    std::uniform_real_distribution<double> distr(0, 1);
...
void Izing::chess(unsigned long int steps, double kT_in, int number_of_threads)
{
    unsigned int start_time = clock();
    // Check number_of_threads
    int max_threads = thread::hardware_concurrency();
    ...
    Number_of_threads = number_of_threads;
    kT = kT_in;
    steps = steps / (Lattice_size * Lattice_size/2);
    Steps = steps;
    layer_hight = Lattice_size / Number_of_threads;
    Magnetizations.resize(Number_of_threads);
    Energy.resize(Number_of_threads);
    ...
    Barrier Bar(Number_of_threads);
    for (int i = 1; i < Number_of_threads; i++)
        threads.push_back(thread(checkmate, i, std::ref(*this), steps, std::ref(Bar)));
    // Use main as 0 thread
    checkmate(0, *this, steps, Bar);
    // Join threads
    for (int i = 0; i < Number_of_threads - 1; i++)
        threads[i].join();
    cout << std::endl;
